  ...
  <div className="flex flex-wrap w-full md:w-3/4 justify-between items-center gap-4">
    ...
    <div className="hidden md:flex items-center gap-2">
      <button
        ...
        className={`px-3 py-2 border rounded shadow-md cursor-pointer transition 
          ${view === "grid" ? "bg-[#d4af37] text-black shadow-lg" : "bg-white hover:bg-[#f5f5f5] text-[#0d1b2a]"} 
      ...
      <button
        ...
        className={`px-3 py-2 border rounded shadow-md cursor-pointer transition 
          ${view === "list" ? "bg-[#d4af37] text-black shadow-lg" : "bg-white hover:bg-[#f5f5f5] text-[#0d1b2a]"} 
          font-body uppercase tracking-wide text-sm`}
      >
              ...
              <MenuItem key={option.name}>
                <button
                  ...
                  className={`block w-full text-left px-4 py-2 text-sm font-body hover:bg-[#f5f5f5] transition ${
                    sortOption === option.name ? "text-[#d4af37] font-semibold" : "text-[#0d1b2a]"
                  }`}
                >
                  {option.name}
                </button>
            ...
            {pageOptions.map((option) => (
              <MenuItem key={option.value}>
                <button
                  ...
                  className={`block w-full text-left px-4 py-2 text-sm font-body hover:bg-[#f5f5f5] transition ${
                    productsPerPage === option.value ? "text-[#d4af37] font-semibold" : "text-[#0d1b2a]"
                  }`}
                >
                  {option.value}
                </button>
